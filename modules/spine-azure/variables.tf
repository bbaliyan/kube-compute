...
variable "cluster_type" {
  description = "Cluster topology intent: 'all_in_one' (control-plane nodes stay schedulable) or 'dedicated_control_plane' (control-plane nodes are tainted so user workloads run only on separate worker pools)."
  type        = string
  default     = "all_in_one"
  validation {
...
variable "resource_group_name" {
  description = "Azure resource group to create every spine resource in (VMs, NICs, NSG, ASGs, load balancer, Key Vault)."
  type        = string
}
...
variable "os_image_urn" {
  description = "Azure Marketplace image URN in Publisher:Offer:SKU:Version format. The tested OS is Ubuntu 26.04 LTS. Null = Ubuntu 26.04 LTS gen2 (x86_64). For arm64, pass an arm64-compatible SKU explicitly."
  type        = string
  default     = null
}
...
variable "availability_zones" {
  description = "Azure availability zones control-plane VMs are spread across, cycling round-robin by index (VM i lands in zone availability_zones[i % length(availability_zones)]). Required to have at least 3 distinct entries when control_plane_count > 1 — enforced at plan time via a resource precondition, matching spine-aws's >= 3 distinct AZ requirement."
  type        = list(string)
  default     = ["1", "2", "3"]
}
