...
run "core_render" {
  ...
  assert {
    condition     = strcontains(nonsensitive(output.cloud_init), "INSTALL_RKE2_VERSION=\"v1.36.1+rke2r1\"")
    error_message = "k8s_version must be injected into the RKE2 install"
  }
}
