...
run "extra_server_manifests_rendered_for_server_init" {
  ...
  assert {
    condition     = strcontains(nonsensitive(output.cloud_init), "/var/lib/rancher/rke2/server/manifests/kube-vip.yaml")
    error_message = "extra_server_manifests keys must render as file paths under RKE2's auto-deploy manifests directory"
  }
  assert {
...
run "extra_server_manifests_absent_for_worker" {
  ...
  assert {
    ...
    error_message = "extra_server_manifests must never render for node_role = worker (a worker has no RKE2 server manifests directory)"
  }
}
