...
variable "cluster_token" {
  description = "Shared secret used to join a server or agent to the cluster (rke2 config.yaml's token:). Required when node_role is server-init, which forms the whole-cluster join secret. Sensitive."
  type        = string
  default     = null
  sensitive   = true
}
...
variable "cluster_agent_token" {
  description = "Separate shared secret accepted only from agents (rke2 config.yaml's agent-token:) — a worker presenting this value can join as an agent but never as a server/etcd member. Required when node_role is server-init. Sensitive."
  type        = string
  default     = null
  sensitive   = true
}
...
variable "registration_address" {
  description = "IP or FQDN of the existing cluster's registration endpoint (a control plane's registration_address output). Used to build config.yaml's server: https://<address>:9345 (RKE2's supervisor/join port, distinct from the 6443 Kubernetes API port) for the agent/server join. Required when node_role is server-join or worker; ignored for server-init."
  type        = string
  default     = null
}

variable "agent_token_fetch_command" {
  description = "Shell command that prints the rke2 agent join token to stdout when run at boot (e.g. a cloud provider's CLI call to fetch a secret from its parameter/secrets store, assembled by a node-pool module). Keeps cloud-init provider-neutral: the caller decides how the token is fetched and delivered to the instance; cloud-init only executes the command it is given. Required when node_role is worker."
  type        = string
  default     = null
}

variable "node_labels" {
  description = "Extra node-label: entries applied at rke2 install time, e.g. { \"topology.kubernetes.io/zone\" = \"eu-west-1a\" }. Provider-neutral: any caller may set arbitrary labels; cloud-init does not interpret the keys."
  type        = map(string)
  default     = {}
}

variable "extra_tls_sans" {
  description = "Additional tls-san: entries for the rke2 server cert (e.g. a registration endpoint's DNS name, or a wildcard hostname). Provider-neutral: any caller may supply arbitrary extra SANs. Only meaningful for node_role = server-init or server-join."
  type        = list(string)
  default     = []
}
...
variable "etcd_snapshot_object_store_bucket" {
  description = "Optional object-store bucket name for uploading etcd snapshots off-node (S3-compatible API — rke2 config.yaml's etcd-s3-bucket:). Null = local-only snapshots. Provider-neutral name: the caller (a control-plane module) resolves this to whatever object store its provider uses."
  type        = string
  default     = null
}
