...
existing `spine-aws` cluster via its join-token flow. One pool = one subnet = one availability
...
security group of its own** in this slice: workers attach only the spine's
...
`k8s_version` (this pool) must not be newer than `spine_k8s_version` (the cluster's control
...
Three inputs come from the spine's outputs (wire them via a terragrunt `dependency` block in a
...
`AmazonSSMManagedInstanceCore`), the same as `spine-aws` — no inbound shell port. IMDSv2 is
...
architecture, matching `spine-aws`.
