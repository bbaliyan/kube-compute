...
run "pool_version_equal_to_spine_is_accepted" {
  command = plan
  override_data {
  ...
  assert {
    ...
    error_message = "equal pool/spine versions must plan successfully"
  }
}

run "pool_version_older_than_spine_is_accepted" {
  command = plan
  override_data {
    target = data.aws_subnet.selected
    values = { availability_zone = "eu-west-1a", vpc_id = "vpc-mock" }
...
run "pool_version_newer_than_spine_is_rejected" {
  command = plan
  override_data {
    target = data.aws_subnet.selected
    values = { availability_zone = "eu-west-1a", vpc_id = "vpc-mock" }
