...
variable "control_plane_node_refs" {
  description = "Control-plane node refs, unmodified from a *-control-plane module's own control_plane_node_refs output (map of node name -> {instance_id, ip, provider}). The orchestrator patches these one at a time — etcd tolerates losing at most one control-plane node at once."
  type = map(object({
    instance_id = string
    ip          = string
    provider    = string
...
variable "worker_node_refs" {
  description = "Worker node refs, unmodified from one or more *-node-pool modules' own worker_node_refs outputs, merged by the caller if there's more than one pool. Defaults to {} for an all_in_one cluster shape with no separate worker pool."
  type = map(object({
    instance_id = string
    ip          = string
    provider    = string
