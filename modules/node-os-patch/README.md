...
- **`tasks/reboot.yml`** — only does anything when `os_patch_reboot_needed` is true
  ...
  come back (`ansible.builtin.reboot` — the ansible-playbook process stays alive
  across the reboot and reconnects on its own over SSH), then, if
  `os_patch_rke2_service` was supplied, waits for that systemd unit to report active
  before considering the step done. `os_patch_rke2_service` is omitted by
