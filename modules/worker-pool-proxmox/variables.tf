...
# ---- Proxmox-specific inputs (mirrors node-proxmox/spine-proxmox) ----
...
variable "spine_k8s_version" {
  description = "The spine's k8s_version. This pool's k8s_version is rejected if it is newer."
  type        = string
}

variable "registration_address" {
  description = "The spine's registration_address output (kube-vip VIP for HA, or the sole node's IP for single-node). Workers join via --server https://<this>:6443."
  type        = string
}
...
variable "cluster_ipset_name" {
  description = "The spine's cluster_ipset_name output. Referenced by name ('+<name>') in this pool's own per-VM firewall rules — the pool never creates or owns the ipset itself."
  type        = string
}
