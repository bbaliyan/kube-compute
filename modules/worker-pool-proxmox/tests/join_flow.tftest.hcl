...
run "worker_gets_agent_token_via_cloud_init_not_ssm" {
  ...
  variables {
    ...
    cluster_ipset_name   = "kube-node-bharat-cluster"
  ...
  assert {
    condition     = alltrue([for k, r in proxmox_virtual_environment_firewall_rules.worker : strcontains(coalesce(r.rule[0].source, ""), "kube-node-bharat-cluster")])
